export class GameBrain {
    ...
    moveExistingPiece(oldX: number, oldY: number, newX: number, newY: number): void{
      if (this.board[oldX][oldY] === this.currentPlayer && this.board[newX][newY] === ""){
  ...
  checkAllLines(board: (string[])[], startX: number, startY: number): string | null {
    const width = 3;
    const height = 3;
  ...
  checkLine(board: (string[])[], startX: number, startY: number, deltaX: number, deltaY: number, length: number): string | null {
    let count = 0;
